...
def is_valid_solution(chromosome):
...
def fitness_function(chromosome):
    return sum(1 for i in range(len(chromosome)) if is_valid_solution(chromosome[:i] + [chromosome[i]] + chromosome[i+1:]))

individualSize, populationSize = 4, 10  # 4-queens problem
i = 0
...
while True:
    ...
    print(f"Generation {i}: Best Chromosome: {instance.population[best_individual][0]}")
    i += 1
